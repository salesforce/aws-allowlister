...
class DoDCCSRGIL5GCQATestCase(unittest.TestCase):
    def test_gh_14_DoDCCSRG_IL5_GC_compliant_services(self):
        results = compliance_data.get_compliant_services(
            db_session=db_session, compliance_standard="DoDCCSRG_IL5_GC"
        )
        expected_results = [
            "config",
            "datasync",
